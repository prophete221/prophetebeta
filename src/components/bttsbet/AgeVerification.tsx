...
export default function AgeVerification() {
    ...
    const checkAge = () => {
      try {
        ...
        if (stored) {
          ...
          if (hoursSince < EXPIRY_HOURS && data.verified) return
        }
        setShow(true)
      } catch {
        setShow(true)
  ...
  const handleConfirm = () => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ verified: true, timestamp: new Date().toISOString() }))
    setShow(false)
  }
