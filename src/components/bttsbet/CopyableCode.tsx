...
export default function CopyableCode({ code, className = '', displayClassName = '' }: { code: string; className?: string; displayClassName?: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 ${className}`} data-promo-code={code}>
      ...
      <motion.button type="button" onClick={copy} whileTap={{ scale: 0.9 }} className="inline-flex min-h-7 items-center gap-1 rounded-md border border-current/20 px-1.5 text-[10px] font-bold transition-colors" style={{ color: copied ? '#ffb800' : 'currentColor' }} aria-label={`Copier ${code}`} aria-live="polite">
